...
$addata=(& type C:\zabbix_agent\scripts\ad).split("`n")
...
#$npd=5
$pdprejson=@{}
#$pddatapattern='(State.*|Reported Channel.*|Array.*|Model.*|Serial number.*|S.M.A.R.T..*|Last Failure Reason.*
#|Aborted Commands.*|Bad Target Errors.*|Ecc Recovered Read Errors.*|Failed Read Recovers.*|Failed Write Recovers .*|Format Errors.*|
#Hardware Errors.*|Hard Read Errors.*|Hard Write Errors.*|Hot Plug Count.*|Media Failures.*|Not Ready Errors.*
#|Other Time Out Errors.*|Predictive Failures.*|Retry Recovered Read Errors.*|Retry Recovered Write Errors.*|Temperature.*)'
$pddatapattern='(State.*|Reported Channel*|Model.*|Serial number.*|S.M.A.R.T..*|Temperature.*)'

#$pddata=((& $arcconf getconfig 1 pd | select-string -Pattern "Device is a Hard drive" -Context 0,20) -replace ">", "").split("`n")
#$pddata=((& type C:\zabbix_agent\scripts\pd | select-string -Pattern "Device is a Hard drive" -Context 0,20) -replace ">", "").split("`n")

$pddata=((& type C:\zabbix_agent\scripts\pd | select-string -Pattern "Device is a Hard drive" -Context 0,27) -replace ">", "").split("`n")
...
$pddata=((& type C:\zabbix_agent\scripts\pd | select-string -Pattern "Device is a Hard drive" -Context 0,26) -replace ">", "").split("`n")
$pddata=(($pddata | Select-String -Pattern $pddatapattern) -split '`n')
$pdinfo=(makeobj($npd)($pddata))
...
$lddata=(& type C:\zabbix_agent\scripts\ld).split("`n")
...
#$lds = "{"+'"data"'+":"+ "["+(($lds -ireplace 'Logical Device name','{#LLDNAME}'))+"]}"
